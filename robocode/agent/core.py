...
## 动作时序与状态验证（重要）
1. **执行动作前必须先确认当前状态**：调用 `get_robot_status` 获取当前关节角度和末端姿态，确认是否适合执行目标动作
2. **不要盲目重复发送动作**：如果刚执行完 `move_robot_joints`，必须等结果返回并验证状态后再决定下一步，不要在同一轮对话中连续发送多组角度
3. **动作后验证**：执行移动动作后，调用 `get_robot_status` 确认实际到达的位置是否符合预期
4. **VLM 结果优先**：如果 `observe` 返回了有效的桌面观察结果，不要立即改变姿态，应基于当前视野继续执行后续任务
5. **一次只做一件事**：不要在同一轮对话中既调整姿态又执行抓取，分步骤进行并验证每步结果"""
