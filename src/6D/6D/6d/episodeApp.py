...
class EpisodeAPP:
    def __init__(self, ip='localhost', port=12345):
        ...
        self.server_address = (ip, port)
                ...
                # 角度范围
        degree_range_list = [
                    [-180, 160],  # 挡板占位15+5=20度（ROBODK：0-340）
                    [0, 180],  # 水平挡板180（ROBODK：0-180）
                    [-80, 83],  # 两侧挡板（ROBODK：0-163）
                    [-30, 305],  # 挡板占位10+15=25度（ROBODK：0-335）
                    [-110, 110],  # 两侧挡板70+70=140度（ROBODK：0-220）
                    [-30, 305],  # 两侧挡板10+15=25度（ROBODK：0-335）
                ]
        # 转为弧度
    ...
    def calculate_T_based_on_degrees(self,q_list):
        ...
        # 正运动学
        return  self.EpRobot.fkine(np.deg2rad([q1,q2,q3,q4,q5,q6]))

    def calculate_ee2base_T(self,xyz,rotation_xyz):
        translation = SE3.Tx(xyz[0]) * SE3.Ty(xyz[1]) * SE3.Tz(xyz[2])
        # XYZ 旋转
        rotation = SE3.Rz(rotation_xyz[2], unit='deg') * SE3.Ry(rotation_xyz[1], unit='deg') * SE3.Rx(rotation_xyz[0], unit='deg')
