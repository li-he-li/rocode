...
class GraspAgent:
    ...
    def grounding_dino(self, image, classes):
        ...
        model = AutoModelForZeroShotObjectDetection.from_pretrained("./grounding-dino-base").to(
        ...
        try:
            while self.running.value:
                    ...
                    for res in results:
                        for box, label, score in zip(res["boxes"], res["labels"], res["scores"]):
                            ...
                            while not self.center_p_queue.empty():
                                try:
                                    self.center_p_queue.get_nowait()
                                except:
                                    break
                ...
                cv2.putText(
                    ...
                    cv2.FONT_HERSHEY_SIMPLEX,
                    1.0,
                    (0, 0, 255) if self.status.value else (0, 255, 0),
                    2,
                )
                cv2.putText(
                    color,
                    f"STATUS: {self.messages[0]}",
                    (20, 90),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.8,
                    (255, 255, 255),
        ...
        while self.running.value:
            ...
            if cls:
                ...
                if pt and self.running.value:
                    ...
                    robot.move_xyz_rotation(
                        approach, [180, 0, 90], rotation_order="xyz", speed_ratio=1
                    )
                    ...
                    robot.move_xyz_rotation(
                        [140, -300, 300], [180, 0, 90], rotation_order="xyz", speed_ratio=1
                    )
                    robot.gripper_off()
