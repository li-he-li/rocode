#!/usr/bin/env python3
"""
Chessboard & Storage Display Demo - MoveIt Version

This tool displays both chessboard and storage areas with YOLO piece detection.
Uses MoveIt for robot motion planning and control.
...
- MoveIt-based robot control with motion planning
- GripperControl service for gripper
...
4. MoveIt and robot running: ros2 launch episode1_urdf_1113_moveit demo.launch.py

Usage:
    python3 14.chess_demo_ros2_moveit.py
...
try:
    ...
    print(f"WARNING: ROS2/MoveIt not available: {e}. Robot control disabled.")
...
class ChessboardDisplay:
    ...
    def process_frame(self, show_grid=False):
        ...
        status = f"Chessboard | YOLO: {'ON' if self.detection_enabled else 'OFF'} | Pieces: {len(detected_pieces)}"
        cv2.putText(
            chess_transformed,
            status,
        ...
        status = f"Storage | Pieces: {len(detected_pieces)}"
        cv2.putText(
            storage_transformed,
            status,
    ...
    def run(self):
        ...
        while True:
            ...
            if key == ord('q'):
                ...
                if self.robot_enabled and not self.robot_busy:
                    print("Moving to home position...")
                    self.robot_busy = True
                    threading.Thread(target=lambda: (self.robot_move_to(self.home_x, self.home_y, self.home_z), setattr(self, 'robot_busy', False)), daemon=True).start()
    ...
    def run_all_displays(self):
        ...
        print("Press 'h' to move robot to home position")
...
# ============================================================
# MoveIt Robot Controller Class
# ============================================================
class MoveItRobotController(Node):
    """ROS2 Node for robot control using MoveIt"""

    def __init__(self):
        super().__init__('chess_demo_moveit_controller')

        # MoveIt configuration parameters
        self.robot_name = 'episode1_urdf_1113'
        self.package_name = 'episode1_urdf_1113_moveit'
        self.planning_group = 'episode_arm'
        ...
        try:
            # Initialize MoveIt
            self.robot = MoveItPy(node_name="chess_moveit_py", config_dict=robot_config)
        ...
        self.get_logger().info('MoveIt Robot Controller initialized')

    def _build_moveit_config(self):
        """Build MoveIt configuration dictionary"""
        base_config = MoveItConfigsBuilder(
            robot_name=self.robot_name,
            package_name=self.package_name
        ).robot_description_semantic(
            "config/episode1_urdf_1113.srdf",
    ...
    def wait_for_servers(self, timeout=5.0):
        ...
        if not self._moveit_ready:
            self.get_logger().error('MoveIt not initialized!')
            return False
    ...
    def move_to_pose(self, x: float, y: float, z: float, x_rot: float, y_rot: float, z_rot: float) -> bool:
        """
        Plan and execute motion to specified pose using MoveIt.

        Args:
            x: X position (mm)
            y: Y position (mm)
            z: Z position (mm)
            x_rot: Rotation around X axis (radians)
            y_rot: Rotation around Y axis (radians)
            z_rot: Rotation around Z axis (radians)

        Returns:
            True if planning and execution successful, False otherwise
        """
        if not self._moveit_ready:
            self.get_logger().error('MoveIt not ready!')
            return False

        # Convert euler angles to quaternion
        q_x, q_y, q_z, q_w = tf_transformations.quaternion_from_euler(x_rot, y_rot, z_rot, axes='sxyz')

        pose_goal = PoseStamped()
        pose_goal.header.frame_id = "base_link"
        pose_goal.pose.position.x = x / 1000.0  # Convert mm to meters
        ...
        self.get_logger().info(f'Moving to: pos=[{x:.2f}, {y:.2f}, {z:.2f}]mm, rot=[{np.rad2deg(x_rot):.1f}, {np.rad2deg(y_rot):.1f}, {np.rad2deg(z_rot):.1f}]deg')
        ...
        if plan_result:
            ...
            time.sleep(0.5)  # Wait for robot state update
    ...
    def move_xyz_sync(self, position, rotation, ik_mode="xyz", speed_ratio=1.0):
        """
        Synchronous move to position - wrapper for move_to_pose.
        Maintains compatibility with existing ChessboardDisplay interface.

        Args:
            position: [x, y, z] in mm
            rotation: [x_rot, y_rot, z_rot] in degrees
            ik_mode: Unused, kept for compatibility
            speed_ratio: Unused for now, MoveIt uses its own velocity scaling
        """
        x, y, z = position
        # Convert rotation from degrees to radians
        x_rot = np.deg2rad(rotation[0])
        y_rot = np.deg2rad(rotation[1])
        z_rot = np.deg2rad(rotation[2])

        return self.move_to_pose(x, y, z, x_rot, y_rot, z_rot)
    ...
    def move_to_target(self, target_name: str) -> bool:
        """
        Plan and execute motion to a named configuration.

        Args:
            target_name: Name of target configuration (defined in SRDF)
        """
        if not self._moveit_ready:
            self.get_logger().error('MoveIt not ready!')
            return False
        ...
        if plan_result:
            self.robot.execute(plan_result.trajectory, controllers=[])
            time.sleep(0.5)
            self.get_logger().info(f'Moved to target: {target_name}')
            return True
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Chessboard & Storage Display Demo (MoveIt Version)')
    ...
    print("=" * 70)
    print(" CHESSBOARD & STORAGE DISPLAY DEMO (MOVEIT VERSION)")
    print("=" * 70)
    print()

    # Initialize ROS2 and MoveIt
    ros2_controller = None
    if ROS2_AVAILABLE and MOVEIT_AVAILABLE:
        try:
            rclpy.init()
            ros2_controller = MoveItRobotController()
            ...
            print(f"WARNING: ROS2/MoveIt initialization failed: {e}")
            import traceback
            traceback.print_exc()
            ros2_controller = None
    else:
        print("WARNING: ROS2/MoveIt not available. Robot control disabled.")
