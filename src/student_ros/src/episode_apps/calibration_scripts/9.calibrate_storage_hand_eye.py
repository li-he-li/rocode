...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QLabel, QVBoxLayout,
                             ...
                             QSpinBox, QTabWidget, QCheckBox, QDialog, QTextEdit)
...
class StorageHandEyeCalibrator(QMainWindow):
    ...
    def __init__(self, robot_node, storage_camera_config: dict, hand_eye_config: dict,
                 camera_config_path: str, hand_eye_config_path: str, camera_index: int = 0):
        super().__init__()
    ...
    def init_ui(self):
        ...
        # IK status label (hidden by default)
        self.ik_status_label = QLabel("")
        self.ik_status_label.setStyleSheet("color: red; font-weight: bold; font-size: 13px; padding: 2px;")
        self.ik_status_label.setAlignment(Qt.AlignCenter)
        self.ik_status_label.setVisible(False)
        pos_layout.addWidget(self.ik_status_label, 3, 0, 1, 2)
        ...
        # Action buttons (2-column grid layout)
        action_layout = QGridLayout()

        self.record_btn = QPushButton(" Record Point (Enter)")
        self.record_btn.setStyleSheet("background-color: #4CAF50; color: white; padding: 8px; font-size: 12px; font-weight: bold;")
        self.record_btn.clicked.connect(self.record_point)
        action_layout.addWidget(self.record_btn, 0, 0)

        self.calc_btn = QPushButton(" Calc T Matrix (C)")
        ...
        action_layout.addWidget(self.calc_btn, 0, 1)
        ...
        action_layout.addWidget(reset_btn, 1, 1)
        ...
        action_layout.addWidget(home_btn, 2, 0)

        self.save_modified_btn = QPushButton("Save Modified Point")
        self.save_modified_btn.setStyleSheet("background-color: #2196F3; color: white; padding: 8px; font-size: 12px;")
        self.save_modified_btn.clicked.connect(self.save_modified_point)
        self.save_modified_btn.setVisible(False)
        action_layout.addWidget(self.save_modified_btn, 2, 1)
    ...
    def move_to_position(self):
        ...
        QTimer.singleShot(100, lambda: self._check_move_result(force=True))
    ...
    def _check_move_result(self, force=False, retries=10):
        """Check move result and show IK status if failed"""
        if self.robot.is_moving() and retries > 0:
            # Still moving, check again later
            QTimer.singleShot(200, lambda: self._check_move_result(force=force, retries=retries - 1))
            return

        self.update_robot_position_display(force=force)

        if not self.robot._last_move_success:
            self.ik_status_label.setText("No IK Solution")
            self.ik_status_label.setVisible(True)
        else:
            self.ik_status_label.setVisible(False)
    ...
    def show_result_dialog(self, result_text):
        """Show calibration results in a popup dialog"""
        dialog = QDialog(self)
        ...
        text_edit.setStyleSheet("padding: 8px; background-color: #f0f0f0; border: 1px solid #ccc;")
        text_edit.setText(result_text)
        layout.addWidget(text_edit)

        close_btn = QPushButton("Close")
        close_btn.setStyleSheet("background-color: #2196F3; color: white; padding: 8px; font-size: 12px;")
    ...
    def calculate_transformation(self):
        ...
        try:
            ...
            self.show_result_dialog(result_text)
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(description="Storage Area Hand-Eye Calibration Tool")
    parser.add_argument('-c', '--camera', type=int, default=0,
                        help='Camera device index (default: 0)')

    parsed_args = parser.parse_args()
    camera_index = parsed_args.camera
