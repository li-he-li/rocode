...
class HandEyeCalibrator(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Action buttons (2x2 grid + extras)
        action_layout = QVBoxLayout()

        action_grid = QGridLayout()

        self.record_btn = QPushButton("Record Point (Enter)")
        self.record_btn.setStyleSheet("background-color: #4CAF50; color: white; padding: 8px; font-size: 12px;")
        self.record_btn.clicked.connect(self.record_point)
        action_grid.addWidget(self.record_btn, 0, 0)

        self.save_modified_btn = QPushButton("Save Modified Pt")
        ...
        action_grid.addWidget(self.save_modified_btn, 0, 0)
        ...
        # Result display (removed from main layout to save space, shown in popup instead)
    ...
    def calculate_transformation(self):
        ...
        try:
            ...
            if max_err > 5.0:
                ...
                msg.setWindowTitle("Warning - High Calibration Error")
                msg.setText(f"Calibration error is high (max={max_err:.2f}mm).\n"
                    "Consider re-teaching the points more carefully.\n"
                    f"Results auto-saved to: {self.config_path}")
                msg.setDetailedText(result_text)
                msg.setStyleSheet("QTextEdit { font-family: Courier; font-size: 10pt; }")
                msg.exec_()
            else:
                msg = QMessageBox(self)
                msg.setIcon(QMessageBox.Information)
                msg.setWindowTitle("Calibration Success")
                msg.setText(f"Calibration complete!\nMean error: {mean_err:.3f}mm\nMax error: {max_err:.3f}mm\n"
                    f"Results auto-saved to: {self.config_path}")
                msg.setDetailedText(result_text)
                msg.setStyleSheet("QTextEdit { font-family: Courier; font-size: 10pt; }")
                msg.exec_()
