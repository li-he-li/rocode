#!/usr/bin/env python3
"""
Simple Chessboard & Storage Display Demo - OpenCV Version
...
- Simple OpenCV display (no PyQt5)
...
class ChessboardDisplay:
    ...
    def __init__(self, camera_config_path, storage_config_path, yolo_weights_path,
                 storage_hand_eye_path=None, chessboard_hand_eye_path=None, camera_index=0):
        """
    ...
    def _send_robot_command(self, command):
        """Send command to robot via TCP socket"""
        try:
    ...
    def robot_move_to(self, x, y, z, speed_ratio=1.0):
        ...
        params = [x, y, z] + self.current_rotation + ["xyz", speed_ratio]
        command = {'action': 'move_xyz_rotation', 'params': params}
        result = self._send_robot_command(command)
    ...
    def robot_gripper(self, enable):
        """Control gripper (True=on/vacuum, False=off/release)"""
        action = 'gripper_on' if enable else 'gripper_off'
        result = self._send_robot_command({'action': action})
        ...
        while True:
            ...
            if key == ord('q'):
                ...
                if self.robot_enabled and not self.robot_busy:
                    print(f"Moving robot to home position: ({self.home_x}, {self.home_y}, {self.home_z})")
                    self.robot_move_to(self.home_x, self.home_y, self.home_z)
                else:
                    print("Robot not available or busy")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Chessboard & Storage Display Demo")
    ...
    print("=" * 70)
    print(" CHESSBOARD & STORAGE DISPLAY DEMO")
    print("=" * 70)
    print()
